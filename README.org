...
| =C-c C-a=     | global  | Insert active file region         |
...
- *Insert file region into input*: Select a region in a file, run
  =M-x pi-coding-agent-insert-region (C-c C-a)=, and pi input gets:
  =@file#L1-L3= + the selected text in a fenced code block.
